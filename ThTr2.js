...
function setup() {
  ...
  // Create down texts
  let downTextSize = 14; // Adjust the size here
  for (let i = 0; i < 30; i++) {
    let message = random(downMessages);
    downTexts.push(new DownText(message, downTextSize));
  }

  // Create up texts
  let upTextSize = 20; // Adjust the size here
  for (let i = 0; i < 30; i++) {
    let message = random(upMessages);
    upTexts.push(new UpText(message, upTextSize));
  }
}
...
class DownText {
  ...
  show(color) {
    textSize(this.size); // Ensure size is applied here
    textAlign(CENTER);
